-- A Premier League fixture can be complete for live autosub purposes before
-- FPL marks it fully final. Use finished_provisional at the final whistle,
-- while retaining the authoritative end-of-Gameweek finalisation process.
       ...
       and not exists (
         ...
         where fixture.gameweek = p_gameweek
           ...
           and not (
             ...
             or coalesce(fixture.is_finished_provisional, false)
           )
       ) then
      if coalesce((
           select stats.minutes from public.player_gameweek_stats stats
...
comment on function public.refresh_provisional_gameweek_autosubs(integer) is
  'Recomputes provisional effective lineups when club fixtures finish provisionally, preserving bench priority, double-Gameweek eligibility and legal formation without finalising the Gameweek.';
