-- Official FPL total_points includes its own defensive-contribution award.
-- Custom leagues replace that award with their configured DEFCON tiers, so
-- retain the official value for scouting while scoring fixtures from a base
-- value with the official DEFCON award removed.

alter table public.player_gameweek_stats
  add column if not exists official_total_points integer,
  add column if not exists official_defcon_points integer not null default 0;

update public.player_gameweek_stats
set official_total_points = total_points
where official_total_points is null;

comment on column public.player_gameweek_stats.total_points is
  'Base FPL points with the official defensive-contribution award removed; custom league DEFCON points are added separately.';

comment on column public.player_gameweek_stats.official_total_points is
  'Unmodified total_points supplied by the official FPL Draft live endpoint.';

comment on column public.player_gameweek_stats.official_defcon_points is
  'Official defensive-contribution points identified in the live scoring explanation and excluded from custom league scoring.';
...
as $$
  ...
  aggregates as (
    select
      stats.player_id,
      sum(coalesce(stats.official_total_points, stats.total_points, 0))::bigint as total_points,
  ...
  form_stats as (
    select
      stats.player_id,
      round(
        sum(coalesce(stats.official_total_points, stats.total_points, 0))::numeric /
...
comment on function public.get_player_pool_current_stats(integer) is
  'Returns official current-season scouting totals while custom league fixture scoring replaces official defensive-contribution awards with league DEFCON tiers.';
